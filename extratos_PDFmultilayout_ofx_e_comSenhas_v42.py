...
def parse_c6(lines):
    ...
    current_year = None
    current_month = None
    month_re = re.compile(r'^(Janeiro|Fevereiro|Março|Marco|Abril|Maio|Junho|Julho|Agosto|Setembro|Outubro|Novembro|Dezembro)\s+(\d{4})\s*\(', re.I)
    date_re = re.compile(r'^\d{2}/\d{2}$')
    value_re = re.compile(r'^[+-]?R\$\s*\d{1,3}(?:\.\d{3})*,\d{2}$|^-R\$\s*\d{1,3}(?:\.\d{3})*,\d{2}$')
    skip_exact = {
        'Data', 'lançamento', 'Data contábil', 'Data lançamento', 'contábil', 'Tipo', 'Descrição', 'Valor', '•',
    ...
    while i < N:
        ...
        m_month = month_re.match(ln)
        if m_month:
            current_month = MESES_PT[m_month.group(1).lower()]
            current_year = int(m_month.group(2))
            i += 1
            continue

        low = ln.lower()
        if ln in skip_exact or low.startswith('entradas: r$') or low.startswith('saídas: r$') or low.startswith('saidas: r$'):
            i += 1
            continue

        if low.startswith('saldo do dia'):
            i += 1
            if i < N and re.match(r'^-?R\$\s*\d', norm_space(lines[i])):
                i += 1
            continue
        ...
        if i + 4 < N and date_re.match(ln) and date_re.match(norm_space(lines[i + 1])) and value_re.match(norm_space(lines[i + 4])):
            dt_lanc = ln
            tipo = norm_space(lines[i + 2])
            desc = norm_space(lines[i + 3])
            valor_txt = norm_space(lines[i + 4])
            try:
                dt = datetime.strptime(f"{dt_lanc}/{current_year}", "%d/%m/%Y").strftime("%d/%m/%Y")
...
def parse_one_pdf(pdf_path):
    ...
    if ("extrato exportado no dia" in txt and "sem lançamentos no mês" in txt and "data contábil" in txt) or name.startswith("c6"):
        return "c6", parse_c6(lines)
