# -*- coding: utf-8 -*-
"""
Extrator multi-layout de extratos bancários (PDF + OFX, sem OCR / sem Tesseract) - v52.2

Uso:
    python extrator_multilayout_consolidado_v52_2_banrisul_layout2.py
ou
    python extrator_multilayout_consolidado_v52_2_banrisul_layout2.py --pasta "C:\\caminho\\dos\\pdfs"
...
def parse_safra(lines):
    """Parser para demonstrativo consolidado Safra/SafraPay.

    Estrutura observada:
      - cabeçalhos mensais com data de referência no formato YYYY/MM/DD;
      - lançamentos com data dd/mm no início da linha;
      - descrição eventualmente em linha anterior ou posterior ao lançamento;
      - Num.Docto. geralmente como token numérico antes do valor;
      - valor com sinal negativo indicado por hífen final;
      - linhas 'CONTA CORRENTE' são saldo e devem ser ignoradas.
    """
    header_year_re = re.compile(r"^\s*(20\d{2})/(\d{2})/(\d{2})\s*$")
    tx_re = re.compile(r"^(?P<dm>\d{2}/\d{2})\s+(?P<body>.+?)\s+(?P<val>\d{1,3}(?:\.\d{3})*,\d{2}-?)\s*$")
    ...
    def is_noise(line: str) -> bool:
        ...
        if not t:
            return True
        if header_year_re.match(t):
            return True
        if low.startswith((
            "banco safra", "cnpj:", "nome:", "data", "descrição", "descricao",
            "num.docto", "déb./créd", "deb./cred", "saldos", "limite", "demonstrativo consolidado",
            "conta:", "set/", "out/", "nov/", "dez/", "jan/", "fev/", "mar/", "abr/", "mai/", "jun/", "jul/", "ago/",
            "continua", "continua.....", "legenda:", "sac -", "ouvidoria -", "previsão para débito", "previsao para debito",
            "valor r$"
        )):
            return True
        if re.fullmatch(r"[A-Z]{3}/\d{4}\s+\d{3}/\d{3}", t):
            return True
        if re.fullmatch(r"\d{3}/\d{3}", t):
            return True
        return False

    def extract_doc_and_desc(body: str):
        body = norm_space(body)
        parts = body.split()
        if len(parts) >= 2:
            cand = parts[-1]
            # Documento típico: 5 a 12 dígitos, sem pontuação; evita capturar pedaços de CPF/CNPJ.
            if re.fullmatch(r"\d{5,12}", cand):
                return norm_space(" ".join(parts[:-1])), cand
        return body, ""

    def should_append_to_previous(prev_desc: str, line: str) -> bool:
        p = norm_space(prev_desc).upper()
        l = norm_space(line).upper()
        if l.startswith("SAFRAPAY"):
            return True
        # Em PIX ENVIADO, pagamento e transferências, a linha seguinte costuma conter contraparte/CPF-CNPJ.
        if p.startswith(("PIX ENVIADO", "PAGAMENTO", "PGTO", "TRANSF", "TED ", "COMPRA CARTAO", "MENSALIDADE", "PREST.")):
            return True
        return False
    ...
    for raw in lines:
        ...
        if is_noise(ln):
            # Cabeçalhos/rodapés quebram prefixos espúrios, mas não removem o último lançamento.
            continue

        mt = tx_re.match(ln)
        if mt and current_year:
            dm = mt.group("dm")
            body = mt.group("body")
            valtok = mt.group("val")
            desc_base, doc = extract_doc_and_desc(body)

            # Saldo diário; não é lançamento.
            if desc_base.upper().startswith("CONTA CORRENTE"):
                pending_prefix = []
                last_idx = None
                continue

            desc_parts = []
            # No layout Safra, a linha anterior costuma ser contraparte apenas para PIX RECEBIDO SAFRAPAY.
            # Para outros históricos, especialmente ANTECIPACAO RV/RESUMO VENDAS, a linha não datada
            # anterior geralmente pertence ao lançamento anterior ou é ruído de extração e não deve contaminar.
            if pending_prefix and desc_base.upper().startswith("PIX RECEBIDO SAFRAPAY"):
                desc_parts.extend(pending_prefix)
            desc_parts.append(desc_base)
            desc = norm_space(" ".join(desc_parts))
            pending_prefix = []

            try:
                dt = datetime.strptime(f"{dm}/{current_year}", "%d/%m/%Y").strftime("%d/%m/%Y")
                val = money_to_float(valtok)
            except Exception:
                last_idx = None
                continue
        ...
        # Linhas não datadas podem ser complemento da transação anterior ou prefixo da próxima.
        if last_idx is not None and should_append_to_previous(rows[last_idx][1], ln):
            rows[last_idx][1] = norm_space(rows[last_idx][1] + " " + ln)
        else:
            # Evita acumular lixo muito distante; no layout real a linha prefixo vem imediatamente antes da data.
            pending_prefix = [ln]
            last_idx = None
...
def parse_banrisul_layout1(lines):
    """Parser Banrisul já existente no extrator.

    Mantido isolado para preservar compatibilidade com o layout antigo,
    cujo mês/ano vem em linha "PERIODO:" / "PERÍODO:".
    """
    mes = None
    ano = None
    for ln in lines:
...
def parse_banrisul_layout2_pdf(pdf_path, pdf_password=None):
    """Parser Banrisul layout II.

    Layout observado:
      B A N R I S U L - DEZEMBRO /2022 - PAG. 1
      DIA OP DOC SIS HISTORICO VALOR SALDO ORIG R*
      01 2584 221604 BPX PIX 99.000,00
          1005 701088 BDX PAGAMENTO TITULO 10.185,51-

    A rotina foi adaptada do script isolado Banrisul_layout_II.py para o
    padrão do extrator universal. Usa pdfplumber porque, nesse layout, a
    ordenação/agrupamento do texto fica mais fiel que a extração simples por
    PyMuPDF. Linhas de saldo/transporte são descartadas pela padronização,
    pois não são operações bancárias.
    """
    if pdfplumber is None:
        return standardize(pd.DataFrame(columns=["Data", "Descrição", "Documento", "Valor"]))
    ...
    try:
        pdf_ctx = pdfplumber.open(pdf_path, password=pdf_password)
    except TypeError:
        # Compatibilidade com versões antigas de pdfplumber.
    ...
    with pdf_ctx as pdf:
        ...
        for pagina in pdf.pages:
            ...
            for linha in linhas:
                ...
                # Detecta competência por "PERIODO:" ou pelo cabeçalho
                # "B A N R I S U L - MÊS /ANO - PAG."
                if "PERIODO:" in linha or "PERÍODO:" in linha:
                    try:
                        periodo = linha.split(":", 1)[1].strip()
                        nome_mes, ano_txt = periodo.split("/")
                        ano = int(re.sub(r"\D", "", ano_txt))
                        mes = meses[nome_mes.strip().upper()]
                        competencia = True
                        continue
                    except Exception:
                        pass
                ...
                if any(x in up for x in [
                    "SALDO", "CPF", "CNPJ", "AGENCIA DESTINO",
                    "CONTA DESTINO", "LIM=", "TAXA=", "CET="
                ]):
                    continue

                partes = linha.split()

                # Formato com dia explícito:
                # 01 2584 221604 BPX PIX 99.000,00
                if re.match(r"^\d{2}\s+\d{4}\s+\d{6}", linha):
                    if len(partes) < 5:
                        continue

                    try:
                        dia_atual = int(partes[0])
                        dia = dia_atual
                        documento = partes[2]
                        sis = partes[3]
                        valor_str = partes[-1]
                        ...
                        historico = " ".join(partes[4:-1])
                    except Exception:
                        continue

                # Formato de continuação sem dia:
                # 1005 701088 BDX PAGAMENTO TITULO 10.185,51-
                elif re.match(r"^\d{4}\s+\d{6}", linha):
                    if dia_atual is None or len(partes) < 4:
                        continue
                    ...
                    try:
                        ...
                        historico = " ".join(partes[3:-1])
                    except Exception:
                        continue

                # Formato mais simples usado por alguns lançamentos do Banrisul.
                else:
                ...
                try:
                    valor = money_to_float(valor_str)
                    data = datetime(year=ano, month=mes, day=dia).strftime("%d/%m/%Y")
                except Exception:
                    continue

                descricao = norm_space((sis + " " + historico).strip())
                rows.append([data, descricao, documento, valor])
...
def parse_banrisul(lines, pdf_path=None, pdf_password=None):
    # Primeiro tenta o layout antigo já estabilizado.
    df_layout1 = parse_banrisul_layout1(lines)
    if not df_layout1.empty:
        return df_layout1

    # Se não houver "PERIODO:" / "PERÍODO:", tenta o layout II.
    if pdf_path:
        return parse_banrisul_layout2_pdf(pdf_path, pdf_password=pdf_password)
...
def detect_itau_statement_year(lines):
    ...
    if m:
        # Em regra, início e fim são do mesmo ano; se houver virada, usa o ano inicial
        # para as datas dd/mm extraídas na sequência.
    ...
    while i < N:
        ...
        if desc and not is_balance_or_summary_line(desc):
            ...
            if not any(x in low for x in [
                "saldo total disponível dia", "saldo total disponivel dia",
                "saldo anterior", "saldo do dia", "saldo final", "saldo parcial",
                "saldo total"
            ]):
                rows.append([dt, desc, "", val])
...
def parse_itau_layout4(lines):
    ...
    Layout observado:
      ...
      - saldo aparece como valor subsequente e deve ser consumido/ignorado;
      - linhas de saldo/aplicação de saldo não devem virar transação.
    """
    year = detect_itau_statement_year(lines)
    date_re = re.compile(r"^\d{2}/\d{2}$")
    money_re = re.compile(r"^-?\d{1,3}(?:\.\d{3})*,\d{2}$")
    ...
    skip_desc_starts = (
        "saldo inicial", "saldo anterior", "saldo final", "saldo parcial",
        "saldo aplic", "saldo em ", "saldo total disponível", "saldo total disponivel"
    )
    ...
    while i < N:
        ...
        desc = norm_space(" ".join(desc_parts))
        low = desc.lower()
        if desc and not low.startswith(skip_desc_starts) and not is_balance_or_summary_line(desc):
            val = money_to_float(norm_space(lines[j]))
            rows.append([dt, desc, "", val])

        # Consome o valor da transação e, quando existir, o saldo imediatamente subsequente.
        j += 1
...
def parse_itau(lines):
    ...
    if ("data lançamentos razão social cnpj/cpf valor (r$) saldo (r$)" in sample or
        ...
        ("lançamentos do período:" in sample and "razão social" in sample and "cnpj/cpf" in sample and "valor (r$)" in sample)):
    ...
    if (("data lançamento ag./origem valor (r$) saldo (r$)" in sample) or
        ...
        ("extrato de " in sample and "ag./origem" in sample and "valor (r$)" in sample and "saldo (r$)" in sample)):
...
def parse_one_pdf(pdf_path):
    ...
    if "banco safra" in txt or "safrapay" in txt or name.startswith("safra"):
        return "safra", parse_safra(lines)
    ...
    if "banrisul" in txt or "b a n r i s u l" in txt or name.startswith("banrisul"):
        return "banrisul", parse_banrisul(lines, pdf_path=pdf_path, pdf_password=pdf_password)
    ...
    # Itaú: alguns PDFs não trazem a palavra "Itaú" no texto extraído pelo PyMuPDF
    # porque o logotipo fica como imagem. Por isso, além do nome/marca,
    # também usamos os cabeçalhos estruturais dos layouts Itaú já mapeados.
    itau_layout3_hint = (
        "lançamentos do período:" in txt and
        "razão social" in txt and
        "cnpj/cpf" in txt and
        "valor (r$)" in txt and
        "saldo (r$)" in txt
    )
    itau_layout4_hint = (
        "extrato de " in txt and
        "ag./origem" in txt and
        "valor (r$)" in txt and
        "saldo (r$)" in txt
    )

    if (
        "itaú" in txt or "itau" in txt or "extrato mensal" in txt or
        name == "itau.pdf" or itau_layout3_hint or itau_layout4_hint
    ):
