# -*- coding: utf-8 -*-
"""
Extrator multi-layout de extratos bancários (sem OCR / sem Tesseract) - v40

Uso:
    python extrator_multilayout_consolidado_v40.py
ou
    python extrator_multilayout_consolidado_v40.py --pasta "C:\\caminho\\dos\\pdfs"
...
def processar_pasta(folder):
    ...
    if not pdfs:
        raise FileNotFoundError(f"Não encontrei PDFs em: {folder}")
    ...
    for idx, pdf_path in enumerate(pdfs, start=1):
        ...
        try:
            layout, df = parse_one_pdf(pdf_path)
            if df.empty:
