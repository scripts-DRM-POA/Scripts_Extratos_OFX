...
# ======================================================
# ?? Utilitário: normalizar TRNAMT (valores monetários)
# ======================================================
def parse_trnamt_to_float(x) -> float | None:
    """
    Converte TRNAMT vindo em formatos inconsistentes para float.

    Exemplos suportados:
      173.300.00 -> 173300.00
      1.343.00   -> 1343.00
      150.00     -> 150.00
      1.234,56   -> 1234.56
      -2.345,67  -> -2345.67

    Regra: se houver mais de um separador ('.' ou ','), o último é decimal e os anteriores são milhar.
    """
    if x is None:
        return None
    s = str(x).strip()
    ...
    # se tiver vários pontos, mantém só o último como decimal
    if s.count(".") > 1:
        parts = s.split(".")
...
def extrair_dataframe_sgml(caminho_ofx: Path) -> pd.DataFrame | None:
    ...
    if 'TRNAMT' in df.columns:
        df['TRNAMT'] = df['TRNAMT'].map(parse_trnamt_to_float)
        ...
        df['TIPO']    = df['TRNAMT'].apply(
            lambda x: 'C' if pd.notna(x) and x > 0 else ('D' if pd.notna(x) and x < 0 else '')
        )
    else:
        df['CREDITO'] = ""
        df['DEBITO']  = ""
...
# ======================================================
# ?? Função 2: Extrair transações para DataFrame (via XML corrigido)
# ======================================================
...
def process_dir() -> Path:
    ...
    for arq in arquivos:
        ...
        if df is not None and not df.empty:
            ...
            print(f"?? Nenhuma transação extraída de: {arq.name}")
    ...
    if not dfs:
        writer.close()
        raise RuntimeError(
            "Nenhum arquivo gerou dados para exportação. "
            "Verifique se os OFX são válidos e/ou se o fallback SGML capturou <STMTTRN>."
        )
